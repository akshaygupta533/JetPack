...
Ring::Ring(float x, float y) {
    ...
    for(int i=0;i<100;i++){
        ...
        vertex_buffer_data[9*i+3] = 1.5*cos(angle);
        vertex_buffer_data[9*i+4] = 1.5*sin(angle);
        vertex_buffer_data[9*i+5] = 0.0;

        vertex_buffer_data[9*i+6] = 1.5*cos(angle+rot_angle);
        vertex_buffer_data[9*i+7] = 1.5*sin(angle+rot_angle);
    ...
    glPointSize(10);
    this->object = create3DObject(GL_TRIANGLES, 100*3, vertex_buffer_data, COLOR_COIN_OUT, GL_POINT);
